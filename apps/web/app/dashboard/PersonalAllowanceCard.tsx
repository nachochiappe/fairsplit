...
import { updatePersonalBudgetPlan, type AppLocale, type PersonalBudgetForecastResponse } from '../../lib/api';
...
export function PersonalAllowanceCard({ forecast, locale, month }: PersonalAllowanceCardProps) {
  ...
  return (
    <section className="overflow-hidden rounded-2xl border border-stroke/80 bg-surface shadow-sm">
      <div className="grid gap-0 lg:grid-cols-[minmax(0,1.05fr)_minmax(320px,0.95fr)]">
        <div className="px-5 py-6 sm:px-6 md:px-8 md:py-8">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div className="min-w-[220px] flex-1">
              <h2 className="text-xl font-semibold tracking-tight text-ink-strong md:text-2xl">{copy.title}</h2>
              <p className="mt-2 max-w-[68ch] text-sm leading-6 text-ink-muted">{copy.description}</p>
            </div>
            <span className="inline-flex min-h-8 shrink-0 items-center gap-2 whitespace-nowrap rounded-full bg-surface-soft px-3 text-xs font-semibold text-brand-700">
              <svg
                aria-hidden="true"
                className="h-4 w-4"
                fill="none"
          ...
          {hasIncome ? (
            <div className="mt-7">
              <p className={`text-sm font-semibold ${hasShortfall ? 'text-rose-700' : 'text-brand-700'}`}>
            ...
            <div className="mt-7 rounded-xl bg-surface-muted px-4 py-4">
                ...
                {allocations.map((item) => (
                  <li className="flex items-center justify-between gap-3" key={item.key}>
                    <span className="flex min-w-0 items-center gap-2 text-ink-muted">
                      <span aria-hidden="true" className={`h-2.5 w-2.5 shrink-0 rounded-full ${item.className}`} />
        ...
        <div className="border-t border-stroke/70 bg-surface-muted/55 px-5 py-6 sm:px-6 md:px-8 md:py-8 lg:border-l lg:border-t-0">
            ...
            <div>
              ...
              <p className="mt-1 text-sm leading-6 text-ink-muted">
                {forecast.historyMonthsUsed === 0
                  ? copy.noHistory
                  : copy.historyUsed(forecast.historyMonthsUsed, forecast.historyMonthsRequested)}
              </p>
            </div>
          ...
          <dl className="mt-6 space-y-3 text-sm">
            ...
            />
            <BudgetLine label={copy.fixedCommitments} locale={locale} value={-Number(forecast.fixedCommitments)} />
            <BudgetLine label={copy.savingsTarget} locale={locale} value={-Number(forecast.savingsTarget)} />
            <BudgetLine label={copy.safetyBuffer} locale={locale} value={-Number(forecast.safetyBuffer)} />
          ...
          <details className="mt-5 border-t border-stroke/70 pt-4 text-sm">
            ...
            <dl className="space-y-3 pb-2 pt-2">
              ...
              <BudgetLine label={copy.alreadyPaid} locale={locale} value={Number(forecast.alreadyPaid)} />
              <BudgetLine
      ...
      {isEditing ? (
        <form className="border-t border-stroke bg-white px-5 py-6 sm:px-6 md:px-8" onSubmit={savePlan}>
          ...
          <div className="mt-6 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
            ...
            />
            <div>
              <label className="text-sm font-semibold text-ink-strong" htmlFor="personal-average-months">
                {copy.lookback}
              </label>
              <input
              ...
              />
              <p className="mt-2 text-xs leading-5 text-ink-muted" id="personal-average-months-help">
                {copy.lookbackHelp}
              </p>
            </div>
          </div>
